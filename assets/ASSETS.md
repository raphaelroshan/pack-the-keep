...
The temporary CC0 breadth kit is documented in [`docs/temporary_asset_kit.md`](../docs/temporary_asset_kit.md) and inventoried in [`assets/temporary/manifest.json`](temporary/manifest.json). It includes Kenney Tiny Dungeon, Tiny Battle, Interface Sounds, RPG Audio, and a curated Particle Pack subset. `0.40.0-actor-readability` integrates temporary board actors, `0.41.0-tactile-audio` routes bounded foley through the semantic cue service, and `0.42.0-combat-vfx` adds restrained response and impact textures. Tactical overlays and procedural/audio fallbacks remain intact. None of these assets are the final Greywatch production direction.
