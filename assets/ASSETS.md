...
The temporary CC0 breadth kit is documented in [`docs/temporary_asset_kit.md`](../docs/temporary_asset_kit.md) and inventoried in [`assets/temporary/manifest.json`](temporary/manifest.json). It includes Kenney Tiny Dungeon, Tiny Battle, Interface Sounds, RPG Audio, and a curated Particle Pack subset. `0.40.0`–`0.43.0` integrate temporary board actors, bounded foley, restrained combat effects, and room-state feedback; `0.44.0-room-accents` uses seven Tiny Dungeon props as low-density Greywatch room-function accents. Tactical overlays and procedural/audio fallbacks remain intact. None of these assets are the final Greywatch production direction.
