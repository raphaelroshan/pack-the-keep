...
The temporary CC0 breadth kit is documented in [`docs/temporary_asset_kit.md`](../docs/temporary_asset_kit.md) and inventoried in [`assets/temporary/manifest.json`](temporary/manifest.json). It includes Kenney Tiny Dungeon, Tiny Battle, Interface Sounds, RPG Audio, and a curated Particle Pack subset. `0.40.0-actor-readability` integrates a small Tiny Battle subset for temporary board actors while retaining the authored tactical overlays and procedural fallback. The remaining files support future layout, animation-timing, and audio-feedback tests; none are the final Greywatch, commander, enemy, or fortress art direction.
