...
| `audio/semantic/*.wav` | 44.1 kHz mono PCM | Fourteen original warning, combat, recovery, control, and terminal cues | Integrated through the bounded semantic cue pool; reproducible via `tools/generate_authored_foley.py` |
...
The temporary CC0 breadth kit is documented in [`docs/temporary_asset_kit.md`](../docs/temporary_asset_kit.md) and inventoried in [`assets/temporary/manifest.json`](temporary/manifest.json). It includes archived Kenney Tiny Dungeon, Tiny Battle, Interface Sounds, RPG Audio, and Particle Pack sources used during prototyping. `0.45.0`–`0.52.0` replace every active room label, actor, room prop, visual effect, and semantic audio cue with original assets. No current presentation profile references the temporary tree; procedural visual and generated-tone fallbacks remain intact.
