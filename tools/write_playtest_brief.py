...
def render_brief(protocol: dict[str, Any], manifest: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "- Copy the matching file from `artifacts/playtest-templates/` and fill its blank session identity, timestamp, device, and scenario fields; or create the record with `tools/new_playtest_session.py` from the source snapshot.",
        "- Each template is bound to this executable and starts every observation as `not_tested`. It is intentionally invalid evidence until a human fills the blank fields.",
