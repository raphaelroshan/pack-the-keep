...
def main() -> int:
    ...
    for index, doctrine in enumerate(data.get("enemy_doctrines", [])):
        if not isinstance(doctrine, dict):
            continue
        references(doctrine.get("response_families", []), {"frontline", "support", "control", "recon", "mobile_response", "morale", "specialist", "controlled_sacrifice", "open_space", "commander_ability", "refuge_plan", "pause", "alternate_lanes", "reserve"}, f"enemy_doctrines[{index}].response_families", errors)
