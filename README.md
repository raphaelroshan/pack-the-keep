...
The repository contains a playable, deterministic vertical slice with a polished main-menu → War Council → Preparation → real-time Battle → Recovery → Results flow. It supports three commanders, three defensive keeps, eleven coherent packs, twenty-one pieces, ten enemy types, twelve doctrines, and fifteen authored scenarios. Save schema 4, settings schema 5, controller navigation, accessibility options, 1280×720 through 2560×1440 layouts, packaged Windows lifecycle checks, deterministic pause/speed behavior, and the complete First Watch tutorial remain covered by automated gates. The all-scenario matrix now covers 126 commander/scenario/seed cases and 252 uninterrupted/save-resumed simulations. The `0.40.0`–`0.44.0` presentation releases established temporary combat art and feedback, `0.45.0`–`0.49.0` replace room labels, every active actor and Greywatch room prop, and every active visual effect, and `0.50.0-ea1-greywatch-anchor` makes the finished anchor an explicit CI-enforced Early Access milestone.
...
Run `python3 tools/validate_early_access_progress.py` to compare the tracked milestone ledger with the current runtime catalog. PTK-EA-1 is complete; overall Early Access readiness remains false while PTK-EA-2 through PTK-EA-6 are planned.
