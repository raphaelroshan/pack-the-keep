...
The repository contains a playable vertical-slice shell and deterministic keep-state foundation. The main scene demonstrates commander selection, scenario selection, pack opening, top-down piece placement, invasion doctrines, wave progress, commander abilities, and saving prototype state. The P0 foundation adds direct map placement with authoritative footprint previews, room/piece/enemy inspection, pack cost and doctrine previews, a one-pack reserve slot, scrollable command controls, and safe load/reset behavior. The P1 slice adds The Warden, Siege Beast area pressure, three authored Greywatch scenarios, escalating doctrine sequences, and bounded cross-process deterministic variation. The P2 pass adds a paused inspection start, a presentation-only 0.5×/1×/2× battle clock, manual one-step control, named keyboard actions, keyboard placement/cancel shortcuts, health and condition bars, explicit state words, declared target lines, a Siege Beast radius cue, transient impact/recovery framing, optional code-generated feedback tones, and high-contrast cues. The P3 pass adds map-first active-enemy selection, deterministic marker hit-testing, Tab/Shift+Tab focus cycling, synchronized dropdown inspection, focused target outlines, a paused response preview, and deterministic recovery-priority advice. P5 recovery work now presents room repair, piece repair, assignment, and assignment clearing as explicit cards with authoritative readiness, costs, benefits, trade-offs, and blocking reasons. Results derive a causal report from wave history and final keep state, including principal pressure, what worked, what failed, and one deterministic replay experiment. The layout summary compares the same fort through Castellan and Warden lenses while exposing floor, zone, room-edge, open-lane, support, assignment, and duplicate-role evidence. P6 now loads the four active packs, both commanders, all eight defensive pieces, all four enemy actors, and all four invasion doctrines from individually validated JSON files through the state-owned content catalog without changing their behavior. The initial combat pass presents a real-time auto-battle over deterministic one-second steps: enemies follow named routes toward behavior targets, defenders automatically counter when their floor, role, placement zone, cooldown, and ammunition allow, ranged defenders consume finite rounds, and surviving ranged defenders reload during recovery. The map reads as a square fort with thick walls, an open courtyard, an open gate, a visible gate approach, and an upper wall walk. Placement previews and piece labels identify `WALL`, `COURTYARD`, or `KEEP`, while placeholder attackers visibly move from the gate toward the courtyard. A generated pixel-art map was attempted but the image-generation quota was unavailable, so this release uses a functional procedural fort renderer rather than claiming final generated art.
...
A later Windows alpha should still add controller support and remapping, user-persistent display scaling, platform adapters, achievements, cloud-safe saves, crash reporting, authored animation sheets, and a fuller soundscape. P2 establishes the keyboard/mouse path, scroll-safe command surface, pause/speed contract, and high-contrast presentation without claiming those later systems are complete. These services should remain outside the core keep simulation.
