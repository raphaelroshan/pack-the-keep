...
This reduces first-run search cost while preserving access to the full command table. Empty preparation disables the quick action until a defender is placed; active Battle keeps the action paused and inspectable; inter-wave Results requires explicit recovery completion before the primary action starts the next authored wave; terminal Results shows the compact three-wave scorecard and restarts the same deterministic preset rather than silently starting a fourth wave. Preparation also exposes a commander-specific layout lens and allows selected-piece removal without refund so compact and open-lane layouts can be compared honestly.
