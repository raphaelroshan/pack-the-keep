...
**Current baseline:** `0.62.0-board-first-recovery`; GPT56, investment, and Early Access gates are machine-checked, all twelve commander/keep geometry fits are authored, Preparation, Assault, and ordinary Recovery are board-first at 1280×720, each phase has truthful decision framing, and all active presentation assets are original. The progress ledgers record exact evidence, automated candidate status, and the owner-controlled distribution boundary.
