...
**Current baseline:** `0.56.0-tactical-labels`; all investment and Early Access gates are machine-checked, first plans remain synchronized with doctrine cards, each phase has truthful decision framing, tactical labels preserve the map-first hierarchy, and all active presentation assets are original. The progress ledgers record exact evidence, automated candidate status, and the owner-controlled distribution boundary.
