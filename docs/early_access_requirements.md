...
**Current baseline:** `0.65.0-preparation-first-viewport`; GPT56, investment, and Early Access gates are machine-checked, all twelve commander/keep geometry fits are authored, Preparation exposes its compact decision rail beside the fortress at 1280×720, Assault, Recovery, and terminal Results remain board-first, each phase has truthful in-world decision framing, and all active presentation assets are original. The progress ledgers record exact evidence, automated candidate status, and the owner-controlled distribution boundary.
