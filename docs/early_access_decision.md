...
The implementation order was Greywatch completion, Ash Ford completion, Twinwatch plus a fourth commander lens, enemy/counter breadth, event/mastery depth, and candidate hardening. PTK-EA-1 through PTK-EA-6 first reached candidate status at `0.51.0-early-access-candidate`; `0.52.0-authored-foley` preserves those gates while removing the last active borrowed presentation family. Human P16 sessions may later calibrate comprehension but were not used as implementation evidence. Headless state tests, deterministic replay, complete-flow fixtures, layout/scaling/input checks, screenshots, and package recovery evidence are the active automated gates; public distribution still requires owner approval.
