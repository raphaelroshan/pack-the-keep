...
**Current release baseline:** `v0.11.1-greywatch-bell-guard`
...
The current slice supports two commanders, fifteen active defender pieces/equipment, eight packs, six enemy types, seven invasion doctrines, six authored scenarios, a two-floor square fort, deterministic six-step combat waves, repair and assignment intervals, finite ranged ammunition, authored armor and armor-piercing counters, nearby ranged support, linked signal redundancy, bounded forecast disruption, commander abilities, pause/speed/manual-step controls, controller navigation/remapping, persistent presentation settings, map-first enemy focus, placement previews, save/load, a quick-playtest flow, and three-wave authored sequences that advance only after explicit recovery completion.

P11 content breadth is in progress. Two teaching pairs are implemented: Crossbow Watch versus Shielded Advance in Red Banner Road, and Bell Guard versus Smoke and Signal in Ash at the Bell. The next bounded objective remains one friendly-doctrine/enemy-question pair, not a broad roster expansion.
