...
**Current release baseline:** `0.16.0-playtest-readiness` on PR #26; remote `main` remains owner-controlled
...
| P16 | Readiness implemented; sessions pending | Controlled alpha protocol, reproducible build identity, evidence schema, and validation are in place. | Run the four human-observed matrix sessions, convert repeated friction into reversible fixes, and require explicit owner approval before release preparation. |
...
Readiness tooling is implemented in `content/p16_playtest_protocol.json`, `tools/new_playtest_session.py`, `tools/validate_p16_playtests.py`, and `playtests/sessions/`. The title screen exposes the exact pre-alpha build identity. The four human-observed matrix sessions and resulting friction fixes remain pending; automation must never populate successful observations or complete the human gate.
