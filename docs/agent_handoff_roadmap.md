...
**Document status:** GPT56 investment packets, Early Access roadmap, authored-foley follow-up, P81 clarity, and P82 combat cadence implemented
**Current development baseline:** `0.70.0-combat-cadence`; PTK-GPT56-1 through PTK-GPT56-5 plus PTK-GPT56-1B and PTK-GPT56-1C are machine-checked, all P81 clarity contracts remain active, and real-time Assault now gives each deterministic tick 1.5 seconds at 1× while separating attackers at shared contact points; optional human evidence and owner distribution approval remain pending
...
The current candidate supports four commanders, twenty-nine active defender pieces/equipment, fifteen packs, twelve enemy types, fourteen invasion doctrines, twenty authored scenarios across three defensive identities, fourteen authored events, two mutually exclusive run modifiers, two-floor boards, deterministic six-tick authored assault phases presented in continuous real time, one-target-per-defender tick commitments, read-only next-tick response previews, transient engagement traces, repair and assignment lulls, finite ranged ammunition, authored armor and armor-piercing counters, nearby ranged support, linked signal redundancy, bounded forecast disruption, prepared breakthrough delay, route visibility, ammunition-aware targeting, adjacent room/piece protection, assigned-specialist priority, four commander abilities, pause/speed/manual-step controls, controller navigation/remapping, persistent presentation settings, responsive 1280×720 through 2560×1440 display support, map-first enemy focus, placement previews, save/load, a quick-playtest flow, and three-phase authored sequences that advance only after explicit recovery completion.

P11 content breadth is complete. Three original teaching pairs are implemented: Crossbow Watch versus Shielded Advance, Bell Guard versus Smoke and Signal, and Shieldwall versus Break the Line. Three Bells at Dusk composes those questions across two viable two-pack baselines. K6 adds Standard Cutters as assigned-specialist hunters. P51.3 adds Road Wardens versus Outriders as an isolated tempo question; P51.4 adds Lantern Watch versus Gloam Knives as an isolated visibility question; P51.5 combines them in The Twilight Road after teaching each alone. K7 exposes fixed seeded variation and replay comparisons, while K8 binds automated private-alpha evidence and limitations into one non-release gate. The Campaign Ledger, Ash Ford, Twinwatch, authored events, accessibility, real-time battle presentation, tutorial, recovery, Results, and packaged lifecycle gates remain active regression contracts. The current local all-scenario matrix runs all nineteen non-overwhelming scenarios for all four commanders across three seeds both uninterrupted and through save checkpoints, proving 228 viable cases and 456 deterministic simulations without treating automation as a human playtest. No human session has been fabricated or inferred; automated status remains a candidate pending human alpha approval.

P5–P66, K3–K8, P54, the responsive-decision follow-up, and PTK-EA-1 through PTK-EA-6 are implemented contracts and should be treated as regression requirements. The next evidence step is P16 human observation when the owner schedules it; those sessions and public distribution approval must not be inferred from automation.
...
| P16 | Playtest kit implemented; sessions pending | Controlled alpha protocol, bundled artifact identity, evidence schema, validation, and repeated-finding summaries are in place. | Human sessions may validate later; explicit owner approval remains mandatory before external distribution. |
| P31 | Implemented; human onboarding evidence pending | First Watch provides a complete game-facing menu-to-victory tutorial with strict objectives, in-world coaching, separate resume data, and per-phase retry checkpoints. | Preserve automated tutorial coverage; use any later human evidence to refine repeated comprehension or pacing friction. |
| P32 | Implemented | Terminal Results preserves the damaged keep beside a dedicated outcome, timeline, causal, damage, consequence, and replay composition with fixed actions. | Preserve as terminal-flow regression coverage. |
| P33 | Implemented | Preparation names the doctrine question, visible layout answer, and one open weakness above the primary assault action. | Strengthen board hierarchy and placement feedback without tactical clutter. |
| P34 | Implemented | A visual registry defines board layer order, ground/upper treatments, critical-room cues, defender role cards, and enemy silhouettes. | Consolidate semantic tones into one focused battle-loop audio service. |
| P35 | Implemented | Presentation-only cues cover assault start, contact, response, impact, breach, recovery, and terminal outcomes while honoring settings and headless operation. | Reconcile ready-beat and later-wave start behavior without changing combat timing. |
| P36 | Implemented | Tick-zero readiness and readable warnings distinguish changed pressure from continuing pressure. | Give inter-wave Recovery a compact what-changed/why/next hierarchy. |
| P37 | Implemented | Recovery states what changed, why it matters, next pressure, action budget, first priority, and trade-off. | Replace raw War Council selectors with compact choice cards. |
| P38 | Implemented | War Council presents commander identity and authored defense pressure through navigable cards with safe fallbacks. | Give Preparation pack offers the same game-facing hierarchy. |
| P39 | Implemented | Preparation exposes doctrine, contents, costs, spatial demand, strength, limitation, question, budget, and offer state in one card. | Simplify the remaining placement and inspection hierarchy. |
| P40 | Implemented | Preparation reads as pack choice, placement/inspection, and defense commitment with diagnostics behind Advanced. | Give room, defender, and threat inspection one consistent action hierarchy. |
| P41 | Implemented | Room, defender, and threat selections share identity, condition, tactical purpose, next action, and detail. | Simplify Battle commands around live state, intervention, and inspection. |
| P42 | Implemented | Battle leads with live state, pause/resume, commander intervention, and focused-threat inspection. | Add opt-in local instrumentation and stable screen-state capture. |
| P43 | Implemented | Local observation snapshots and a stable nine-screen capture harness are available without network telemetry. | Extract a presentation snapshot boundary without changing simulation ownership. |
| P44 | Implemented | Battle renders from one deterministic read-only snapshot for state, time, availability, focus, and response preview. | Add a debug-only presentation audit overlay. |
| P45 | Implemented | `--debug-ui` reports viewport, focus owner, major regions, and clipping counts without intercepting input. | Add consistent Escape/back paths and unsaved-progress language. |
| P46 | Implemented | Escape/controller-back navigation and unsaved-change confirmation are deterministic and predictable. | Use automated evidence and targeted visual review to address remaining friction. |
...
| P55 | Implemented | The board established swappable actor textures for melee, ranged, heavy, and siege recognition beneath the existing tactical overlays. | Preserve the texture/fallback boundary while P62-era authored silhouettes replace the temporary set. |
| P56 | Implemented | Existing semantic cues established a bounded four-player sample pool, mute/effects-volume behavior, and generated-tone fallback. | Superseded by P66 original samples; preserve the playback boundary. |
| P57 | Implemented | Resolved defender responses and hostile melee, ranged, and demolition impacts use restrained textures beneath procedural marks and tactical overlays. | Historical temporary-art proof replaced by P65. |
| P58 | Implemented | Damaged and breached rooms gain state-driven atmosphere, while successful room and defender repairs create one localized restored-value pulse. | Preserve the P65-authored effect hierarchy during human play. |
...
| P65 | Implemented | Eight original 48×48 marks replace active Particle Pack combat, room-damage, breach, and repair textures while preserving effect timing, tint, reduced-motion behavior, and procedural fallback. | Preserve the presentation-only boundary. |
| P66 | Implemented | Fourteen reproducible original mono WAV cues replace active temporary Interface Sounds/RPG Audio samples while preserving semantic IDs, pooling, mute/volume behavior, headless operation, and generated-tone fallback. | Use human P16 listening evidence to tune the mix without changing simulation authority. |
...
| P71 | Implemented | Recovery stably places every legal action before blocked diagnostics while retaining exact explanations and keeping Finish Recovery last. | Preserve action legality and controller-first focus as recovery options evolve. |
| P81 | Implemented | Recovery, War Council, Assault, Results, Settings, and scenario capture use player-facing, state-truthful language and evidence paths at 1280×720 and 2560×1440. | Evaluate battle pacing and actor congestion as a separate mechanics-risk slice. |
| P82 | Implemented | Normal-speed assaults last roughly nine seconds across six deterministic ticks, effects scale inside each interval, and crowded attackers retain a readable contact formation with only the focused badge on the board. | Use real P16 observation to tune cadence before changing balance. |
