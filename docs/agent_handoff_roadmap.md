...
**Current release baseline:** `0.16.3-playtest-kit` on the current P16 branch; remote `main` is CI-green and owner-controlled
...
Readiness tooling is implemented in `content/p16_playtest_protocol.json`, `tools/write_playtest_build_manifest.py`, `tools/new_playtest_session.py`, `tools/validate_p16_playtests.py`, `tools/summarize_p16_playtests.py`, and `playtests/sessions/`. CI bundles a `playtest-build.json` manifest beside the Windows executable; the session generator rejects an executable whose name, size, hash, version, or source metadata differ. Matrix completion is evaluated per artifact cohort so evidence from different builds cannot be combined. Stable issue keys appearing in two or more human records become deterministic task candidates; automation must never invent findings, populate successful observations, or complete the human gate. The four human-observed matrix sessions and resulting friction fixes remain pending.
