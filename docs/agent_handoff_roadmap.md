...
**Current release baseline:** `0.15.0-second-defensive-identity` on PR #26; remote `main` remains owner-controlled
...
| P15 | Implemented baseline | Greywatch and Ash Ford now provide distinct room graphs, spatial rules, pack doctrine, recovery profiles, and deterministic defense-loop coverage. | Add only the bounded regional consequence slice next; do not build a campaign economy. |
