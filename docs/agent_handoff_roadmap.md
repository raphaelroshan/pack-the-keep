...
**Document status:** Active planning contract for post-v0.13.1 development
**Current release baseline:** `0.13.1-event-ledger` on PR #26; remote `main` remains `0.12.3-alpha-packaged-pause-close`
...
| P13 | In progress | `workshop_can_wait` is integrated into Gatehouse Lock, and the existing Ledger/Results expose a bounded newest-first event history plus stable run flags. | Next build one bounded forecast → decision → consequence chain. |
