...
**Document status:** Automated game-quality roadmap implemented through P57 combat effect readability
**Current development baseline:** `0.42.0-combat-vfx`; the board layers temporary licensed actors and restrained response/impact textures beneath the existing deterministic tactical grammar, while the semantic cue service uses bounded temporary CC0 foley with generated-tone fallback; every K1 responsive, simulation, accessibility, packaging, and recovery gate remains active, and optional human private-alpha evidence and owner distribution approval remain pending
