...
**Document status:** Automated game-quality roadmap implemented through P61 room-label legibility
**Current development baseline:** `0.45.0-readable-rooms`; every compact room uses a stable fully fitted board label above Greywatch's low-density temporary room props, temporary licensed actors, restrained combat effects, state-driven damaged-room atmosphere, localized repair confirmation, and the existing deterministic tactical grammar; every K1 responsive, simulation, accessibility, packaging, and recovery gate remains active, and optional human private-alpha evidence and owner distribution approval remain pending
