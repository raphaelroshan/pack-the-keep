...
**Document status:** Active planning contract for post-v0.13.0 development
**Current release baseline:** `0.13.0-workshop-recovery` on PR #26; remote `main` remains `0.12.3-alpha-packaged-pause-close`
...
| P13 | In progress | `workshop_can_wait` is integrated into Gatehouse Lock with typed recovery effects, persistence, deterministic replay, UI smoke, and a visual checkpoint. | Next connect bounded event history to the existing Ledger and Results. |
