...
| Performance budget | PASS: 40 runs in 2533 ms; 120 large-text UI refreshes in 130 ms |
...
| Room-function accents | PASS: seven Greywatch functional rooms resolve loadable temporary CC0 props; the inner yard, outer wall, Ash Ford, and Twinwatch remain unaccented; high contrast preserves geometry and simulation state |
...
| Authored core actor readability | PASS: four defender-role and four signature-enemy SVG silhouettes resolve from stable original paths; extended enemies retain a declared CC0 fallback; tactical overlays and simulation state remain unchanged |
...
The complete `scripts/verify.sh` suite passed locally after the room-label update. The existing packaged Windows and human hardware gates remain separate: CI must still build and exercise the Windows candidate, and no human observation or distribution approval is claimed here.
