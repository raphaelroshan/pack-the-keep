...
| Performance budget | PASS: 40 runs in 1982 ms; 120 large-text UI refreshes in 122 ms |
...
The complete `scripts/verify.sh` suite passed locally after the authored battle-effect update. The existing packaged Windows and human hardware gates remain separate: CI must still build and exercise the Windows candidate, and no human observation or distribution approval is claimed here.
