...
**Current baseline:** `0.27.1-recovery-results-distinction`
...
Implement K6 next. Add exactly one complete teaching-content slice: one
commander lens, keep identity, pack, or enemy family with a clear question,
counter, weakness, authored scenario fixture, visual states, deterministic
balance coverage, and player-facing explanation. Preserve at least two viable
answers, all existing save and accessibility contracts, and the completed
presentation boundaries. Do not bundle unrelated content, and do not convert
automated metrics into human evidence.
```
