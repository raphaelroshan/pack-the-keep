...
## 2026-09-05 execution checkpoint — PTK-GPT56-1C implemented

Build `0.68.0-assault-threat-dossier` closes the pause/inspection gap without changing combat. Board-first Assault now projects the focused threat's health, doctrine, target, route, next strike, committed defender response, counter, intervention state, and next command together beside the fortress. Fresh 1280×720 and 1600×900 Greywatch sequences prove tick-zero readiness, post-contact pause, one intervention, damage, both Recovery lulls, repair feedback, all three assault phases, and causal Results. Two seeded openings and all-phase save/resume remain covered by their deterministic gates.
