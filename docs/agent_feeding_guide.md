...
> Current baseline override for the compact prompt below: `0.53.0-investment-vertical`, including completed PTK-I1 through PTK-I6 and PTK-EA-1 through PTK-EA-6, distinct visible first plans for all three keeps, four commanders, three six-scenario keeps, fifteen packs, twenty-nine pieces, twelve enemies, fourteen events, the completed original presentation program, the automated K8 hardening gate, and real packaged process-termination recovery evidence.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.53.0-investment-vertical`: four commanders, fifteen packs, twenty scenarios distributed across three six-scenario keeps, twelve enemy types, fourteen authored events, distinct keep-authored first plans, deterministic continuous multi-phase combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, a polished menu-to-results journey, original active presentation assets, and completed PTK-I1 through PTK-I6 plus PTK-EA-1 through PTK-EA-6. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.53.0-investment-vertical`; PTK-I1 through PTK-I6, PTK-EA-1 through PTK-EA-6, and the automated K1–K8, P51–P66, P54, responsive-decision, room-label, authored-actor, authored-room, authored-effect, authored-audio, and repair-feedback gates are complete. Human sessions must be recorded only through the P16 protocol when the owner schedules them.
