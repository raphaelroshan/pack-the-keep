...
> Current baseline override for the compact prompt below: `0.25.5-responsive-layout`, including the P44 Battle presentation snapshot, P45 debug-only layout auditing, P46 confirmed unsaved-run navigation, P47 self-contained tagged playtest cohorts, and P48 responsive War Council/Preparation composition.
...
The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.25.5-responsive-layout`; issue the game-quality feeds in this order. Automated verification, deterministic replay, full-flow launches, controller/scaling checks, and screenshot review are the active gates. Human sessions are optional validation and must not block implementation or controlled content breadth once the automated and visual contracts pass.

### Current feed 1 — P49 keep-first visual identity pass

```text
Keep the fortress visible while browsing packs, placing defenders, inspecting rooms or units, and committing the defense. Replace the highest-noise procedural Greywatch board surfaces with a coherent warm tactile top-down treatment. Preserve all board geometry, placement coordinates, room graphs, state colors, condition bars, target lanes, responsive breakpoints, high-contrast cues, large-text layout, reduced motion, and authoritative state. Produce an asset manifest, before/after captures at 1280x720 and 2560x1440, and run the complete verification wrapper. Do not add units, enemies, campaign systems, or decorative effects that reduce tactical readability.
```
