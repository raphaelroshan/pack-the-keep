...
> Current baseline override for the compact prompt below: `0.35.0-twilight-road`, including the completed P51 controlled-content sequence, the automated K8 hardening gate, and real packaged process-termination recovery evidence.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.35.0-twilight-road`: three commanders, eleven packs, fifteen scenarios across Greywatch, Ash Ford, and Twinwatch, ten enemy types, deterministic continuous multi-phase combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, a polished menu-to-results journey, the First Watch tutorial, and the completed K1–K8, P51, and P54 automated gates. Quartermaster adds reserve economy; Twinwatch adds paired-post defense; Road Wardens versus Outriders teaches prepared tempo; Lantern Watch versus Gloam Knives teaches visibility; The Twilight Road combines those proven questions with two viable plans. The next bounded task is one P52 replay variation or recovery trade-off without grind or a forced build. Human P16 evidence remains pending. Keep simulation separate from UI, preserve deterministic seeds and save compatibility, add tests before polish, and never infer public-distribution approval.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.35.0-twilight-road`; the automated K1–K8, P51, and P54 gates are complete. The next bounded task is one P52 replay variation or recovery trade-off. Human sessions are optional validation and must be recorded only through the P16 protocol when the owner schedules them.
