...
> Current baseline override for the compact prompt below: `0.65.0-preparation-first-viewport`, including completed PTK-GPT56-1 through PTK-GPT56-5 plus PTK-GPT56-1B, the 4x3 commander-to-geometry matrix, a compact 1280×720 Preparation decision rail, board-first Assault, Recovery, and terminal Results, in-world player guidance, four commanders, three keeps, fifteen packs, twenty-nine pieces, twelve enemies, twenty scenarios, fourteen events, and the complete automated/package gate.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.65.0-preparation-first-viewport`: PTK-GPT56-1 through PTK-GPT56-5 and PTK-GPT56-1B are complete, all twelve commander/keep pairings expose an authored geometry fit, and Preparation, Assault, Recovery, and terminal Results keep their board and immediate decision or verdict together at 1280×720. Narrow Preparation also keeps commander, pack doctrine, forecast, first plan, and Ready Defense legible without initial scrolling. The existing four commanders, fifteen packs, twenty scenarios, twelve enemy types, fourteen events, deterministic continuous combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, original active presentation assets, and package gates remain enforced. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.65.0-preparation-first-viewport`; PTK-GPT56-1 through PTK-GPT56-5, PTK-GPT56-1B, PTK-I1 through PTK-I6, PTK-EA-1 through PTK-EA-6, P67–P77, and the automated K1–K8, P51–P66, P54, responsive-decision, room-label, authored-actor, authored-room, authored-effect, authored-audio, repair-feedback, player-language, and preparation-first-viewport gates are complete. Human sessions must be recorded only through the P16 protocol when the owner schedules them.
