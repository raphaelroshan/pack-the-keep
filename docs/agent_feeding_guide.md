...
> Current baseline override for the compact prompt below: `0.26.0-keep-first-visual-finish`, including P48 responsive War Council/Preparation composition and P49 authored Greywatch surfaces plus on-board room/defender selection projection.
...
The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.26.0-keep-first-visual-finish`; issue the game-quality feeds in this order. Automated verification, deterministic replay, full-flow launches, controller/scaling checks, and screenshot review are the active gates. Human sessions are optional validation and must not block implementation or controlled content breadth once the automated and visual contracts pass.

### Current feed 1 — K3 presentation-panel extraction
