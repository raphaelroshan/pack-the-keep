...
> Current baseline override for the compact prompt below: `0.42.0-combat-vfx`, including completed P51 content, P52 replayable Twilight mastery, P53 hardening, the responsive-decision pass, temporary licensed actors, foley, and combat effects, the automated K8 hardening gate, and real packaged process-termination recovery evidence.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.42.0-combat-vfx`: three commanders, eleven packs, fifteen scenarios across Greywatch, Ash Ford, and Twinwatch, ten enemy types, ten authored events, deterministic continuous multi-phase combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, a polished menu-to-results journey, the First Watch tutorial, and the completed K1–K8, P51–P57, and P54 automated gates. Temporary CC0 actor sprites, bounded semantic foley, and restrained response/impact textures reinforce the existing tactical grammar; none is final production work. No mechanics roadmap milestone remains. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.42.0-combat-vfx`; the automated K1–K8, P51–P57, P54, responsive-decision, temporary-actor, semantic-audio, and combat-effect gates are complete. Human sessions are the remaining evidence layer and must be recorded only through the P16 protocol when the owner schedules them.
