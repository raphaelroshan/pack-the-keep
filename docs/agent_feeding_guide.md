...
> Current baseline override for the compact prompt below: `0.50.0-ea1-greywatch-anchor`, including completed PTK-EA-1, P51 content, P52 replayable Twilight mastery, P53 hardening, the responsive-decision pass, fully fitted room labels, original defender, enemy, Greywatch room, combat, room-state, and repair visuals, temporary licensed foley, the automated K8 hardening gate, and real packaged process-termination recovery evidence.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.50.0-ea1-greywatch-anchor`: three commanders, eleven packs, fifteen scenarios across Greywatch, Ash Ford, and Twinwatch, ten enemy types, ten authored events, deterministic continuous multi-phase combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, a polished menu-to-results journey, the First Watch tutorial, and the completed PTK-EA-1, K1–K8, P51–P65, and P54 automated gates. Original small-scale board visuals, fully fitted room labels, and bounded temporary semantic foley reinforce the existing tactical grammar. PTK-EA-2 is next. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.50.0-ea1-greywatch-anchor`; PTK-EA-1 plus the automated K1–K8, P51–P65, P54, responsive-decision, room-label, authored-actor, authored-room, authored-effect, semantic-audio, and repair-feedback gates are complete. PTK-EA-2 is the next implementation milestone. Human sessions must be recorded only through the P16 protocol when the owner schedules them.
