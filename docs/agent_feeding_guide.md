...
> Current baseline override for the compact prompt below: `0.70.0-combat-cadence`, including completed PTK-GPT56-1 through PTK-GPT56-5 plus PTK-GPT56-1B, PTK-GPT56-1C, P81, and P82, the 4x3 commander-to-geometry matrix, a focused-threat Assault dossier, truthful Recovery guidance, replay-first Results, readable nine-second normal-speed assaults, deterministic contact formations, four commanders, three keeps, fifteen packs, twenty-nine pieces, twelve enemies, twenty scenarios, fourteen events, and the complete automated/package gate.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.70.0-combat-cadence`: PTK-GPT56-1 through PTK-GPT56-5, PTK-GPT56-1B, PTK-GPT56-1C, P81, and P82 are complete. Preserve the truthful decision surfaces and the six-tick deterministic simulation; 1× presentation maps each tick to 1.5 real seconds, exchange effects fit inside that interval, and contact formations are visual only. The existing four commanders, fifteen packs, twenty scenarios, twelve enemy types, fourteen events, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, original active presentation assets, and package gates remain enforced. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.70.0-combat-cadence`; PTK-GPT56-1 through PTK-GPT56-5, PTK-GPT56-1B, PTK-GPT56-1C, PTK-I1 through PTK-I6, PTK-EA-1 through PTK-EA-6, P67–P82, and the automated K1–K8, P51–P66, P54, responsive-decision, player-language, interface-hierarchy, assault-dossier, gameplay-clarity, and combat-cadence gates are complete. Human sessions must be recorded only through the P16 protocol when the owner schedules them.
