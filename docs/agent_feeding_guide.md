...
> Current baseline override for the compact prompt below: `0.46.0-authored-core-actors`, including completed P51 content, P52 replayable Twilight mastery, P53 hardening, the responsive-decision pass, fully fitted room labels, original core actor silhouettes, temporary licensed room accents and extended enemies, foley, combat effects, and room/recovery feedback, the automated K8 hardening gate, and real packaged process-termination recovery evidence.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.46.0-authored-core-actors`: three commanders, eleven packs, fifteen scenarios across Greywatch, Ash Ford, and Twinwatch, ten enemy types, ten authored events, deterministic continuous multi-phase combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, a polished menu-to-results journey, the First Watch tutorial, and the completed K1–K8, P51–P62, and P54 automated gates. Original small-scale core actors, fully fitted room labels, and temporary CC0 room props, extended enemies, bounded semantic foley, combat effects, and room/recovery feedback reinforce the existing tactical grammar. No mechanics roadmap milestone remains. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.46.0-authored-core-actors`; the automated K1–K8, P51–P62, P54, responsive-decision, room-label, authored-core-actor, temporary-room-accent, semantic-audio, combat-effect, and repair-feedback gates are complete. Human sessions are the remaining evidence layer and must be recorded only through the P16 protocol when the owner schedules them.
