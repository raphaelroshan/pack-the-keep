...
> Current baseline override for the compact prompt below: `0.38.0-alpha-hardened`, including completed P51 content, P52 replayable Twilight mastery, the P53 end-to-end accessibility/persistence/controller/audio audit, the automated K8 hardening gate, and real packaged process-termination recovery evidence.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.38.0-alpha-hardened`: three commanders, eleven packs, fifteen scenarios across Greywatch, Ash Ford, and Twinwatch, ten enemy types, ten authored events, deterministic continuous multi-phase combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, a polished menu-to-results journey, the First Watch tutorial, and the completed K1–K8, P51–P53, and P54 automated gates. The final P53 integration journey composes large text, high contrast, reduced motion, muted semantic cues, controller-first focus, two file-backed save boundaries, seeded pressure, responsive terminal Results, and the existing package/failure-recovery gates. No autonomous roadmap milestone remains. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.38.0-alpha-hardened`; the automated K1–K8, P51–P53, and P54 gates are complete. Human sessions are the remaining evidence layer and must be recorded only through the P16 protocol when the owner schedules them.
