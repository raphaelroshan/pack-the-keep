...
> Current baseline override for the compact prompt below: `0.59.0-gpt56-packet-completion`, including completed PTK-GPT56-1 through PTK-GPT56-5, the 4x3 commander-to-geometry matrix, synchronized first plans, four commanders, three keeps, fifteen packs, twenty-nine pieces, twelve enemies, twenty scenarios, fourteen events, and the complete automated/package gate.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.59.0-gpt56-packet-completion`: PTK-GPT56-1 through PTK-GPT56-5 are complete, all twelve commander/keep pairings expose an authored geometry fit, and the existing four commanders, fifteen packs, twenty scenarios, twelve enemy types, fourteen events, deterministic continuous combat, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, original active presentation assets, and package gates remain enforced. Human P16 evidence must be recorded only from real scheduled sessions, and public distribution still requires explicit owner approval. Keep simulation separate from UI and preserve every completed gate.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.59.0-gpt56-packet-completion`; PTK-GPT56-1 through PTK-GPT56-5, PTK-I1 through PTK-I6, PTK-EA-1 through PTK-EA-6, P67–P71, and the automated K1–K8, P51–P66, P54, responsive-decision, room-label, authored-actor, authored-room, authored-effect, authored-audio, and repair-feedback gates are complete. Human sessions must be recorded only through the P16 protocol when the owner schedules them.
