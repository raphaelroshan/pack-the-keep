...
> Current baseline override for the compact prompt below: `0.27.1-recovery-results-distinction`, including deterministic screen snapshots, eight-beat Battle presentation, explicit Recovery sacrifice, and a causal-first terminal debrief.
...
The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.27.1-recovery-results-distinction`; issue the game-quality feeds in this order. Automated verification, deterministic replay, full-flow launches, controller/scaling checks, and screenshot review are the active gates. Human sessions are optional validation and must not block implementation or controlled content breadth once the automated and visual contracts pass.
...
### Current feed 1 — K6 controlled content slice
