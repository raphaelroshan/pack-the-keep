...
> Current baseline override for the compact prompt below: `0.27.0-battle-beat-readability`, including deterministic read-only snapshots for all major screens and an eight-beat Battle presentation grammar with speed-scaled and reduced-motion forms.
...
The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.27.0-battle-beat-readability`; issue the game-quality feeds in this order. Automated verification, deterministic replay, full-flow launches, controller/scaling checks, and screenshot review are the active gates. Human sessions are optional validation and must not block implementation or controlled content breadth once the automated and visual contracts pass.
...
### Current feed 1 — K5 Recovery and Results distinction
