...
> Current baseline override for the compact prompt below: `0.34.0-lantern-watch`, including the first four P51 controlled-content slices, the completed automated K8 hardening gate, and real packaged process-termination recovery evidence.

```text
You are the lead implementation agent for Pack the Keep. Read `design/design_prompt.md`, `README.md`, `AGENTS.md`, `docs/decision_log.md`, `docs/agent_handoff_roadmap.md`, and the relevant design card before editing. The current baseline is `0.34.0-lantern-watch`: three commanders, eleven packs, fourteen scenarios across Greywatch, Ash Ford, and Twinwatch, ten enemy types, deterministic continuous multi-phase combat, unit-first targeting with explicit demolition roles, save schema 4, settings schema 5, 2560x1440 support, controller/accessibility coverage, a polished menu-to-results journey, the First Watch tutorial, and the completed K1–K8 plus P54 automated gates. Quartermaster adds reserve economy; Twinwatch adds paired-post defense; Road Wardens versus Outriders teaches prepared tempo; Lantern Watch versus Gloam Knives teaches prepared visibility while preserving melee interception. The next bounded task is one combined challenge using both proven P51 enemy questions with at least two viable answers. The human-authored P16 evidence protocol remains pending. Keep simulation separate from UI, preserve deterministic seeds and save compatibility, add tests before polish, and report intent, plan, changed files, exact verification, risks, and one next task. Never fabricate human observations or infer approval for public distribution or storefront release.
```

## Current post-P31 feed order

The original feeds below describe the historical build-up from the prototype and should not be issued as unimplemented tasks. The current baseline is `0.34.0-lantern-watch`; the automated K1–K8 sequence and P54 forced-close package gate are complete. P51.1–P51.4 are implemented; the next bounded task is one combined Outrider/Gloam Knife challenge with at least two viable answers. Human sessions are optional validation and must be recorded only through the P16 protocol when the owner schedules them.
