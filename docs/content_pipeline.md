...
| `data/events/*.json` | Runtime phase triggers, choices, requirements, typed effects, and follow-up links. |
| `data/modifiers/*.json` | Runtime progression unlocks, information effects, and explicit starting trade-offs. |
...
| `tools/validate_runtime_content.py` | Validator for runtime commander, piece, pack, enemy, doctrine, scenario, event, and modifier schemas, stable filenames/IDs, references, typed operations, event scheduling/graph rules, and manifest parity. The event contract is declared in `content/event_schema.json`. |
