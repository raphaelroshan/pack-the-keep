...
**Current build identity:** `0.70.0-combat-cadence` — all prior gates remain enforced; normal-speed Assault now maps six authoritative ticks to roughly nine seconds, attack effects scale within each interval, crowded contact formations separate actor silhouettes, and non-focused badges defer to the timeline and roster. Human sessions and owner distribution approval remain pending.
...
Stage defender melee/ranged responses and hostile melee/ranged/demolition impacts. Confirm each uses a distinct original effect while exact health trails, damage values, target lines, focus, and beat labels remain dominant. Damage and breach a room, then repair a room and defender; confirm the two atmosphere marks and localized repair pulse resolve from original assets. Toggle reduced motion and high contrast, and confirm effect timing, geometry, simulation state, and procedural fallbacks remain unchanged.
...
Prepare melee and ranged defenders, then enter Battle against melee, ranged, and demolition threats. Confirm response and impact textures remain brief and subordinate to health trails, damage labels, target lines, focus rings, and room state. Toggle reduced motion, pause, manual-step, and each speed. Confirm missing effect textures leave procedural projectile, slash, ring, and damage feedback intact. Verify the `0.41.0` foley acceptance checks as a regression. All borrowed actors, effects, and sounds remain temporary CC0 testing material rather than final Greywatch production work.

## v0.41.0 acceptance checks

With feedback audio enabled, confirm warning, contact, response, impact, breach, recovery, and terminal cues are distinct and restrained; then verify mute and each effects-volume preset. Missing samples must retain generated-tone fallback.
...
Select The Twilight Road with Road Wardens plus Crossbow Watch. After phase two, confirm **One Road Before Nightfall** leads the Recovery command rail and controller focus. Choose **Carry lamp oil to the stair**, confirm exactly one recovery action remains, and verify the final forecast and roster show delayed Outriders from the placed Stake Line plus revealed Gloam Knives from the one-shot lamp preparation. Replay with Lantern Watch plus Runner Network and choose **Replant road stakes** to prove the opposite mixed answer. Save/resume at the open decision and during phase three must preserve the chosen branch exactly.
