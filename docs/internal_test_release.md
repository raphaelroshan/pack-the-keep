...
**Current build identity:** `0.11.1-greywatch-bell-guard` — two P11 teaching pairs covering armor and signal disruption

## Purpose

This package is an internal pre-alpha test release, not a commercial demo or storefront build. Its purpose is to test whether Greywatch supports more than one viable strategic lens and whether the battle now communicates decisions clearly: choose the Castellan or Warden, select one of six authored scenarios, preview and reserve a limited pack offer, place units directly on either floor with a footprint preview, inspect rooms/pieces/enemies, read an escalating forecast, pause or run the wave at three presentation speeds, advance manually, use Lockdown or Rally, inspect health, armor, signal state, and combat metrics, and recover after pressure.
