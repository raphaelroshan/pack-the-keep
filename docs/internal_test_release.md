...
**Current build identity:** `0.33.0-road-wardens` — the development candidate adds the Road Wardens/Outrider tempo teaching pair to the Quartermaster and Twinwatch P51 baseline; automated K8 and forced-close gates remain complete, while human sessions and owner approval remain pending

## Purpose

This package is an internal pre-alpha test release, not a commercial demo or storefront build. Its purpose is to test whether Greywatch, Ash Ford, and Twinwatch support distinct strategic lenses and whether the battle communicates decisions clearly: choose the Castellan, Warden, or Quartermaster, select one of thirteen authored scenarios, preview difficulty, enemy roster, pressure phases, recommended packs, and end-state rules, place units directly on either floor with a footprint preview, inspect rooms/pieces/enemies, read an escalating forecast, watch each assault unfold continuously at three presentation speeds, pause or step for inspection, use Lockdown, Rally, or Resupply, inspect health, armor, signal state, breakthrough momentum, protection, assigned-specialist priority, and combat metrics, recover during authored lulls, or reach an explicit terminal collapse in The Last Bell.

## v0.33.0 acceptance checks

Select Before the Horn and compare Road Wardens against Crossbow Watch. Confirm an unopposed Outrider reports live momentum and tick-two contact, while a living Stake Line beside an authored route room reports `Charge: DELAYED` and tick-three contact. Verify Hook Guard, concentrated Crossbow fire, and open-lane Runner Pair remain visible answers; save/resume must preserve the effective timing exactly. The packaged P12/K8 lifecycle and forced-close recovery audits must continue to pass.
...
Select The Cut Standard in War Council and confirm the concrete seeded material, morale, and applicable room-pressure variation is visible above **Enter Keep**. Complete the scenario with a partial doctrine answer and confirm terminal Results names the same seed pressure, covered and uncovered doctrines, recovery actions used versus capacity, opened packs, and a replay experiment aimed at the first uncovered doctrine. Save/load and accessibility settings must not change that summary.
...
Open War Council, Preparation, Battle, inter-wave Recovery, and terminal Results. Confirm each screen retains its existing commands, controller focus, responsive layout, tutorial gates, and player-facing content. Rebuild each presentation snapshot twice from unchanged state and confirm byte-equivalent output with no serialized-state change. Confirm cards and debrief panels render the exact retained snapshot, save/load remains unchanged, and no presentation field enters the replay key.

## v0.26.0 acceptance checks

Enter Greywatch Preparation and confirm the ground fort and upper wall use distinct authored stone/timber texture regions beneath the existing rooms, connections, pieces, labels, health bars, and placement guides. Click the Gate and Pike Squad; each must receive a clear board outline and a compact plate naming identity, condition, purpose, and next action. Enable high contrast and confirm texture noise recedes while frames and selection outlines strengthen. Confirm Ash Ford retains its river-specific fallback and all board coordinates, clicks, placement, combat, and saves remain unchanged.

## v0.25.5 acceptance checks

Launch on a 1280×720 display with the saved 1600×900 window preference and confirm the window fits and centers inside the usable display. At 1280×720, War Council and Preparation must stack the main surface above the command rail without horizontal clipping; the selected commander/scenario context and **Enter Keep** action remain reachable. At 1600×900 and 100%, confirm the intentional two-column composition remains. At 125% and 150% UI scale, confirm compact cards/navigation, controller focus, the Preparation strategic brief, fortress, pack context, and **Ready Defense** action remain reachable without mutating run state.
...
The release starts with The Castellan, The Warden, or The Quartermaster, ten packs, three defensive identities, and thirteen authored scenarios. The persistent gameplay board keeps placement and combat overlays on the fortress rather than replacing it. Battle is a real-time presentation of a deterministic auto-battle: enemies move continuously along named routes, resolve behavior targets at authoritative tick boundaries, and attack on contact while defenders automatically counter according to role, floor, assignment, cooldown, ammunition, nearby support, armor tags, signal integrity, breakthrough momentum, and adjacent protection. Road Wardens add Hook Guard and Stake Line; Before the Horn isolates the Outrider's early unit-hunter pressure and shows whether its contact remains at tick two or is delayed to tick three. Preparation permits scenario selection, bounded pack opening, pack preview/reserve, direct grid placement, room assignment, and repair. Battle presents readable routes, targets, health, special states, doctrine labels, target lines, transient engagement traces, causal damage reports, and map-first focus selection. Clicking an active enemy marker selects it; Tab and Shift+Tab cycle active enemies; `E` focuses the current threat. Results return the player to a recovery lull after a Hold or Partial Breach. New assaults begin live; Space pauses/resumes, `1`/`2`/`3` select 0.5×/1×/2×, `N` advances one tick while paused, `R` arms placement, Escape cancels placement, `M` mutes code-generated feedback tones, and `C` toggles high-contrast cues. Pike Squad, Runner Pair, and Hook Guard are melee and use no ammunition; Fire Team, Fire Brazier, and Crossbow Patrol are ranged and consume finite rounds; surviving ranged defenders reload when recovery closes.
