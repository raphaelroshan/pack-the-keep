...
**Current build identity:** `0.67.0-settings-first-watch-hierarchy` — PTK-GPT56-1 through PTK-GPT56-5, PTK-GPT56-1B, and all prior gates remain enforced; 1280×720 War Council and Settings expose complete decisions, First Watch opens as a staged briefing, and Preparation retains its compact commander/doctrine forecast, selected pack, first plan, and Ready Defense action beside the fortress while all-phase save/replay, cross-keep continuation, accessibility, package, forced-close, and original-asset evidence remain green; human sessions and owner distribution approval remain pending
