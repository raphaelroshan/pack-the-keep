...
**Current integration:** `0.70.0-combat-cadence` uses original small-scale SVG silhouettes for every active actor, Greywatch room-function props, combat/room/recovery effects, and fourteen original semantic WAV cues. Earlier visual and audio kits remain archived as licensed historical material, but no current presentation profile references them. Stable fitted room labels, room fills, health bars, bounded status badges, deterministic contact spacing, procedural combat marks, and generated tones remain the fallbacks.
