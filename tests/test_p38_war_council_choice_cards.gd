...
func _initialize() -> void:
	...
	_check(String(panel.scenario_identity_label.text).contains("Open Lanes turns Greywatch") and String(panel.scenario_identity_label.text).contains("Recommended pack: Firekeepers"), "changing commander should update the read-only geometry recommendation")
	...
	ui._refresh_war_council_cards()
	_check(String(panel.scenario_identity_label.text).contains("cannot copy Greywatch") and String(panel.scenario_identity_label.text).contains("marked crossing"), "Ash Ford should explain why the compact Greywatch plan cannot be copied")
