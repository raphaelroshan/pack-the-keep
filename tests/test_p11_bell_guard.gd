...
func _initialize() -> void:
	...
	_check(int(status.get("pack_count", 0)) == 8 and int(status.get("piece_count", 0)) == 15 and int(status.get("enemy_count", 0)) == 6 and int(status.get("doctrine_count", 0)) == 7 and int(status.get("scenario_count", 0)) == 6, "P11 second-pair catalog counts should be complete")
