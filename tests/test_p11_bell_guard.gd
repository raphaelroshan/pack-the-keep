...
func _initialize() -> void:
	...
	_check(int(status.get("pack_count", 0)) == 9 and int(status.get("piece_count", 0)) == 17 and int(status.get("enemy_count", 0)) == 7 and int(status.get("doctrine_count", 0)) == 8 and int(status.get("scenario_count", 0)) == 7, "P11 catalog counts should include all teaching pairs")
