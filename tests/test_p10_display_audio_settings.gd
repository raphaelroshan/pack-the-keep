...
func _initialize() -> void:
	...
	ui._set_window_size(1)
	ui._toggle_fullscreen()
	ui._set_effects_volume(1)
	ui._toggle_mute()
	_check(ui.window_size_index == 1 and ui.fullscreen_enabled, "display controls should retain the selected windowed size while fullscreen is active")
	...
	_check(restored.window_size_index == 1 and restored.fullscreen_enabled and restored.effects_volume_index == 1, "a new UI should restore display and effects-volume preferences")
	...
	file.close()
	restored._load_preferences()
	_check(not restored.fullscreen_enabled and restored.window_size_index == 0 and restored.effects_volume_index == 3, "schema-2 settings should migrate with documented display and audio defaults")
