...
func _initialize() -> void:
	...
	_check(bool(first.content_catalog_status().get("ok", false)) and int(first.content_catalog_status().get("pack_count", 0)) == 9 and int(first.content_catalog_status().get("commander_count", 0)) == 2 and int(first.content_catalog_status().get("piece_count", 0)) == 17 and int(first.content_catalog_status().get("enemy_count", 0)) == 7 and int(first.content_catalog_status().get("doctrine_count", 0)) == 8 and int(first.content_catalog_status().get("scenario_count", 0)) == 8 and int(first.content_catalog_status().get("event_count", 0)) == 9 and int(first.content_catalog_status().get("modifier_count", 0)) == 2, "KeepState did not expose the complete P13 runtime catalog")
