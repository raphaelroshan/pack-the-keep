...
func _test_castellan_and_basic_roster() -> void:
	...
	_expect(keep.enemy_ids().size() == 12 and keep.enemy_ids().has("battering_ram") and keep.enemy_ids().has("harrier"), "Early Access should expose twelve enemy families")
...
func _test_enemy_attack_cadence_projection() -> void:
	var keep: PackKeepState = PackKeepState.new(3307)
	keep.place_piece("pike_squad", Vector2i(0, 3), "ground")
	keep.start_wave("distributed_sabotage")
	var serialized_before: String = JSON.stringify(keep.serialize())
	var raider_approach: Dictionary = keep.enemy_attack_timing(0)
	var sapper_approach: Dictionary = keep.enemy_attack_timing(1)
	_expect(int(raider_approach.get("attack_interval", 0)) == 1 and int(raider_approach.get("next_attack_step", 0)) == 2, "Raider cadence should project its contact tick before arrival")
	_expect(int(sapper_approach.get("attack_interval", 0)) == 2 and int(sapper_approach.get("next_attack_step", 0)) == 3, "Sapper cadence should project its authored contact tick before arrival")
	_expect(JSON.stringify(keep.serialize()) == serialized_before, "attack cadence inspection must not mutate authoritative state")
	keep.advance_wave(2.0)
	var raider_contact: Dictionary = keep.enemy_attack_timing(0)
	_expect(bool(raider_contact.get("in_contact", false)) and int(raider_contact.get("next_attack_step", 0)) == 3, "one-tick Raider cadence should advance to the next strike after contact")
	keep.advance_wave(1.0)
	var sapper_contact: Dictionary = keep.enemy_attack_timing(1)
	_expect(bool(sapper_contact.get("in_contact", false)) and int(sapper_contact.get("next_attack_step", 0)) == 5, "two-tick Sapper cadence should expose its next valid strike")
	keep.advance_wave(0.5)
