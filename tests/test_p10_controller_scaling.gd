...
func _initialize() -> void:
	...
	_check(saved_payload is Dictionary and int(saved_payload.get("schema_version", 0)) == 5 and saved_payload.get("input_bindings") is Dictionary, "controller and scale preferences should persist in the current settings schema")
